    ...
    public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand) {
        ...
        if (player.rotationPitch > 70) {
            for(int i = 0; i < 5; i++) {
                ...
                if (this.spawnFangs(player.getPosX() + (double)MathHelper.cos(rotatedYaw) * 1.5D, headY, player.getPosZ() + (double)MathHelper.sin(rotatedYaw) * 1.5D, standingOnY, rotatedYaw, 0, world, player)) hasSucceeded = true;
            }
            for(int k = 0; k < 8; k++) {
                float rotatedYaw = yawRadians + (float)k * (float)Math.PI * 2.0F / 8.0F + 1.2566371F;
                if (this.spawnFangs(player.getPosX() + (double)MathHelper.cos(rotatedYaw) * 2.5D, headY, player.getPosZ() + (double)MathHelper.sin(rotatedYaw) * 2.5D, standingOnY, rotatedYaw, 3, world, player)) hasSucceeded = true;
            }
        } else {
            for(int l = 0; l < 16; l++) {
                double d2 = 1.25D * (double)(l + 1);
                if (this.spawnFangs(player.getPosX() + (double)MathHelper.cos(yawRadians) * d2, headY, player.getPosZ() + (double)MathHelper.sin(yawRadians) * d2, standingOnY, yawRadians, l, world, player)) hasSucceeded = true;
            }
        }
