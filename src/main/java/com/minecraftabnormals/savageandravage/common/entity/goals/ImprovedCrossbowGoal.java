...
public class ImprovedCrossbowGoal<T extends CreatureEntity & IRangedAttackMob & ICrossbowUser> extends Goal {
    ...
    public void tick() {
        ...
        if (shouldMoveTowardsEnemy) {
            ...
            else this.mob.getNavigation().moveTo(this.blockPos.getX(), this.blockPos.getY(), this.blockPos.getZ(), speedChange);
        } else this.mob.getNavigation().stop();

        if (target != null)
            this.mob.getLookControl().setLookAt(target, 30.0F, 30.0F);
        else this.mob.getLookControl().setLookAt(this.blockPos.getX(), this.blockPos.getY(), this.blockPos.getZ(), 30.0F, 30.0F);
    ...
    private boolean hasFirework() {
        ...
        else for (ItemStack projectileStack : CrossbowItem.getChargedProjectiles(this.mob.getItemInHand(ProjectileHelper.getWeaponHoldingHand(this.mob, item -> item instanceof CrossbowItem)))) {
