...
public class SREntities {
	...
	public static final RegistryObject<EntityType<IceChunkEntity>> ICE_CHUNK = ENTITIES.register("ice_chunk", () -> EntityType.Builder.<IceChunkEntity>create(IceChunkEntity::new, EntityClassification.MISC).size(2.2F, 1.0F).func_233606_a_(8).func_233608_b_(Integer.MAX_VALUE).build(SavageAndRavage.MODID + ":iceologer_ice_chunk"));
	...
	private static boolean isSpawnableBiome(Biome biome) {
		return biome.getCategory() == Biome.Category.MUSHROOM || biome.getCategory() == Biome.Category.NONE || biome == ForgeRegistries.BIOMES.getValue(new ResourceLocation("biomesoplenty:rainbow_valley"));
	}
