...
import com.teamabnormals.savageandravage.core.other.SREvents;
...
public class ConfusionBoltEntity extends ThrowableProjectile {
	private static final EntityDataAccessor<Integer> TICKS_TILL_REMOVE = SynchedEntityData.defineId(ConfusionBoltEntity.class, EntityDataSerializers.INT);

	public ConfusionBoltEntity(EntityType<? extends ConfusionBoltEntity> type, Level world) {
		super(type, world);
		this.setNoGravity(true);
	}

	public ConfusionBoltEntity(Level world, LivingEntity thrower, int ticksTillRemove) {
		super(SREntities.CONFUSION_BOLT.get(), thrower, world);
		this.setNoGravity(true);
		this.entityData.set(TICKS_TILL_REMOVE, ticksTillRemove);
	}

	public ConfusionBoltEntity(Level world, double x, double y, double z, int ticksTillRemove) {
	...
	protected void onHitEntity(EntityHitResult result) {
		...
		if (owner != null && entity instanceof LivingEntity) {
			...
			if (livingEntity.isAffectedByPotions()) {
				livingEntity.addEffect(new MobEffectInstance(SREffects.WEIGHT.get(), 140, 2));
