...
public class SRLootModifierProvider extends LootModifierProvider {
	...
	public SRLootModifierProvider(PackOutput output, CompletableFuture<Provider> provider) {
	...
	protected void registerEntries(Provider provider) {
		...
		this.entry("entities/creeper").selector(new ConditionedResourceSelector(new NamesResourceSelector("entities/creeper"), new ConfigValueCondition(SavageAndRavage.location("config"), SRConfig.COMMON.creepersDropSporesAfterExplosionDeath, "creepers_drop_spores_after_explosion_death", Maps.newHashMap(), false))).addModifier(new LootPoolsModifier(Collections.singletonList(LootPool.lootPool().name(this.modId + ":creeper_explosion_drops").setRolls(ConstantValue.exactly(1.0F)).when(DamageSourceCondition.hasDamageSource(DamageSourcePredicate.Builder.damageType().tag(TagPredicate.is(DamageTypeTags.IS_EXPLOSION)))).add(LootItem.lootTableItem(SRItems.CREEPER_SPORES.get()).apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 3.0F)))).build()), false));
	}
}