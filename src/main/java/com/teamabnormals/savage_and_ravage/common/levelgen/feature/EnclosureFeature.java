	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (origin != null) {
			...
			if (!clearOutlinePositions.isEmpty()) {
				Griefer griefer = SREntityTypes.GRIEFER.get().create(level.getLevel());
				if (griefer != null) {
	...
	private void generateHole(ArrayList<BlockPos> holePositions, int minY, WorldGenLevel reader, Random rand) {
		for (BlockPos holePos : holePositions) {
			...
			if (rand.nextFloat() < 0.3f) {
				Mob entity = rand.nextFloat() < 0.5f ? EntityType.CREEPER.create(reader.getLevel()) : SREntityTypes.CREEPIE.get().create(reader.getLevel());
				if (entity != null) {
